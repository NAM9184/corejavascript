...
function first(){
    let x = 10;
    // outer enviroment reference 외부환경 참조로 인해서 x 의 값을 second가 참조하여 적용
    function second(){
        let y = 20;
        return x + y;
    }
    ...
    //=======================
    //결국 단순하게 return 30과 동일
}



//  안쪽 함수를 꺼내 바깥 환경에서 사용할 수 있다. 그것이 ♥♥♥♥closure♥♥♥♥

// currying function
// first()()
// 함수 내부에 위치한 내장함수까지 실행시키기 위한 방법


const result =  first(); 
//first() 에 담겨있는 것은 30의 값이 아닌
//second 함수 본문이기 때문에 result를 콘솔에 대입해도 second 본문만 나올뿐이다.
//그럼 여기서 result는 무엇인가 => 함수가 된것이고 result()를 입력하면 결과로 확인가능하다.


// 클로져를 한마디로 정의하자면 *****함수는 태어난 환경을 기억한다******
...
counter()
// counter를 실행하면 본문을 새롭게 수집하여 =>  새롭게 정의하니까 값이 증가하는 것이 아닌 1이 계속해서 반환된다.


const c1 = counter();
// counter 함수를 c1에 담아서 반복실행시키면 값이 증가하는 이유는 *실행할때마다* 새롭게 counter함수 환경을 생성하기 때문에
// 가비지컬렉터의 역할이 수핻되지 않아서 값이 증가하는 것을 결과로 확인하게 되는 것이다.

const c2 = counter();
// 이때 새롭게 c2함수가 생성이 된다면 이또한 counter함수 내부에 존재하는 구문을 포함하지만, c1의 값을 계승하거나 복사하지않고,
// 똑같이 별개의 함수로서 새로운 환경을 추가로 생성한다 
// const c3 =counter();
// const c4 =counter();
// const c5 =counter();
// const c6 =counter(); ......
