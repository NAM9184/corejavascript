...
// 1. shopOrder 는 객체로 shopOrder의 menu에 접근해야하기 때문에 shopOrder.menu를 통해 접근
// 2. shopOrder의 forEach문(각각 반복한다)를 사용하여 첫번째 배열에 접근하여 계산을 하고,
// 3. 2번째 메뉴의 접근한 값을 total 값에 적용하여 산출한다.


const result = shopOrder.menu.reduce((acc,cur) => acc + cur.price * cur.count ,0);

// reduce를 사용하여 메뉴의 총 금액을 계산해주세요.
...
// 정말정말 다시 생각해 봐야 한다 내가 새롭게 등록한 객체가 실제로 사용한 방법인지 올바른지 확인해야한다.